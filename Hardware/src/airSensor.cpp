...
AirSensor::AirSensor(uint8_t dhtPin, uint8_t dhtType, HardwareSerial& serial) 
    : dht(dhtPin, dhtType), pmsSerial(serial), pms(serial) {
        // BODY
}
...
SensorData AirSensor::readAll() {
    ...
    if (isnan(data.temperature) || isnan(data.humidity)) {
        data.isValid = false; // Báo cờ lỗi nếu mất kết nối DHT
    }
    ...
    if (pms.readUntil(pmsData)) {
        ...
        data.pm25 = 0;
        data.isValid = false; // Báo cờ lỗi nếu không đọc được UART
    }
